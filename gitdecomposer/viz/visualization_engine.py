...
class VisualizationEngine:
    ...
    def create_contributor_analysis_charts(self, save_path: Optional[str] = None) -> go.Figure:
        ...
        contributor_activity = (
            self.metrics.contributor_analyzer.get_contributor_activity_over_time()
        )
        ...
        if not contributors.empty:
            ...
            fig.add_trace(go.Bar(x=names, y=commits, name="Commits"), row=1, col=1)

            # Top contributors by lines added
            lines_added = [c["total_insertions"] for c in top_10_commits]
            fig.add_trace(
                go.Bar(x=names, y=lines_added, name="Lines Added", marker=dict(color="lightgreen")),
                row=1,
                col=2,
            )
        ...
        if not contributor_activity.empty:
            ...
            if "author" in contributor_activity.columns:
                ...
                for author in contributor_activity["author"].unique()[:5]:  # Top 5 for readability
                    ...
                    if (
                        "first_commit_date" in author_data.columns
                        and not author_data["first_commit_date"].isna().all()
                    ):
                        # Create a simple representation
                        fig.add_trace(
                            go.Scatter(
                                x=[
                                    author_data["first_commit_date"].iloc[0],
                                    author_data["last_commit_date"].iloc[0],
                                ],
                                y=[
                                    author_data["total_commits"].iloc[0],
                                    author_data["total_commits"].iloc[0],
                                ],
                                mode="lines+markers",
                                name=author,
        ...
        fig.update_layout(title="Repository Contributor Analysis", height=800, showlegend=True)
    ...
    def create_file_analysis_visualization(self, save_path: Optional[str] = None) -> go.Figure:
        ...
        if file_extensions:
            ...
            fig.add_trace(go.Pie(labels=extensions, values=counts, name="Extensions"), row=1, col=1)
    ...
    def create_enhanced_file_analysis_dashboard(self, save_path: Optional[str] = None) -> go.Figure:
        ...
        if file_churn and "churn_over_time" in file_churn:
            ...
            if window_size > 1:
                ...
                fig.add_trace(
                    go.Scatter(
                        x=dates, y=churn_values, mode="lines", name="Daily Churn", opacity=0.6
                    ),
                    row=2,
                    col=1,
                )
                fig.add_trace(
                    go.Scatter(
                        x=dates, y=moving_avg, mode="lines", name="Trend", line=dict(width=3)
                    ),
                    row=2,
                    col=1,
                )
        ...
        if doc_coverage:
            if "coverage_by_type" in doc_coverage:
                ...
                fig.add_trace(
                    go.Pie(labels=types, values=coverage, name="Doc Coverage", hole=0.3),
                    row=2,
                    col=2,
                )
        ...
        try:
            ...
            if directory_stats:
                ...
                fig.add_trace(
                    go.Bar(x=dirs, y=dir_changes, name="Directory Activity"), row=3, col=2
                )
