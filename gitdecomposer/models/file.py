...
class FileInfo:
    ...
    def is_test_file(self) -> bool:
        ...
        return (self.file_type == FileType.TEST or
                'test' in self.path.lower() or
                'spec' in self.path.lower())
...
class CodeQuality:
    ...
    def needs_refactoring(self) -> bool:
        ...
        return (self.cyclomatic_complexity > 15 or
                self.cognitive_complexity > 20 or
                self.code_duplication > 10 or
                self.technical_debt_ratio > 0.3)
...
class FileNetwork:
    ...
    def highly_coupled_files(self) -> List[str]:
        ...
        return [
            file for file, score in self.cohesion_scores.items()
            if score > 0.7
        ]
...
class CodeOwnership:
    ...
    ownership_distribution: Dict[str, Dict[str, float]] = field(default_factory=dict)  # file -> {contributor: percentage}
    ...
    def get_files_by_owner(self, owner: str) -> List[str]:
        ...
        return [
            file for file, file_owner in self.primary_owners.items()
            if file_owner == owner
        ]
