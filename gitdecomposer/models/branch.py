...
class BranchingStrategy:
    ...
    def is_git_flow(self) -> bool:
        ...
        return (self.develop_branch is not None and
                self.feature_prefix is not None and
                self.release_prefix is not None)
    
    @property
    def is_github_flow(self) -> bool:
        """Check if using GitHub Flow strategy."""
        return (self.develop_branch is None and
                self.main_branch in ['main', 'master'])
...
class BranchCollaboration:
    ...
    def high_collaboration_branches(self) -> List[str]:
        ...
        return [
            branch for branch, contributors in self.shared_branches.items()
    ...
    def collaboration_health_score(self) -> float:
        ...
        if self.conflict_patterns:
            avg_conflicts = sum(len(conflicts) for conflicts in self.conflict_patterns.values()) / len(self.conflict_patterns)
...
class BranchProtection:
    ...
    def high_risk_branches(self) -> List[str]:
        ...
        return [
            branch for branch, violations in self.rule_violations.items()
            if len(violations) >= 3
        ]
