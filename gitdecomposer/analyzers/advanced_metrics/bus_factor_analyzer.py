...
class BusFactorAnalyzer(BaseMetricAnalyzer):
    ...
    def get_description(self) -> str:
        return ("Minimum number of people whose absence would severely impact "
                "project continuity based on knowledge distribution.")
    
    def calculate(self, 
                 lookback_months: int = 6,
                 knowledge_threshold: float = 0.8,
                 decay_half_life: int = 90) -> Dict[str, Any]:
        """
        ...
        knowledge_weights = self._calculate_knowledge_weights(
            lookback_months, decay_half_life
        )
        ...
        if not knowledge_weights:
            return {
                'bus_factor': 0,
                'knowledge_weights': {},
                'error': 'No commits found in the specified timeframe'
            }
        
        # Step 2: Find minimum contributor set covering target knowledge
        bus_factor, coverage_analysis = self._calculate_bus_factor(
            knowledge_weights, knowledge_threshold
        )
    ...
    def _calculate_knowledge_weights(self, 
                                   lookback_months: int, 
                                   decay_half_life: int) -> Dict[str, Dict[str, float]]:
    ...
    def _calculate_bus_factor(self, 
                            knowledge_weights: Dict[str, Dict[str, float]], 
                            threshold: float) -> Tuple[int, Dict[str, Any]]:
        """Calculate the actual bus factor from knowledge weights."""
        total_knowledge = sum(
            sum(files.values()) for files in knowledge_weights.values()
        )
        ...
        if total_contributors > 0:
            ...
            if concentration_ratio < 0.3:
                recommendations.append(
                    "Consider spreading knowledge more evenly across team members"
                )
