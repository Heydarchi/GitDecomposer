...
class BranchLifecycleAnalyzer(BaseMetricAnalyzer):
    ...
    def get_description(self) -> str:
        return ("Analyzes the complete lifecycle of feature branches from "
    ...
    def calculate(self, include_active_branches: bool = False) -> Dict[str, Any]:
        ...
        if not lifecycle_data:
            return {
                'lifecycle_data': [],
                'analysis': {},
                'error': 'No branches found for lifecycle analysis'
            }
    ...
    def _analyze_single_branch(self, branch, is_merged: bool = True) -> Dict[str, Any]:
        ...
        if is_merged:
            ...
            if merge_commit:
                ...
                total_lifecycle_time = merge_commit.committed_datetime - branch_created if branch_created else timedelta(0)
            else:
                completion_time = timedelta(0)
                total_lifecycle_time = last_commit.committed_datetime - branch_created if branch_created else timedelta(0)
        else:
            completion_time = timedelta(0)
    ...
    def _perform_lifecycle_analysis(self, lifecycle_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        total_lifecycle_times = [ld['total_lifecycle_hours'] for ld in lifecycle_data if ld['total_lifecycle_hours'] > 0]
        commit_counts = [ld['commit_count'] for ld in lifecycle_data]
        commit_frequencies = [ld['commit_frequency_per_day'] for ld in lifecycle_data if ld['commit_frequency_per_day'] > 0]
